...
function saveConfig(config: Config): void {
  const lines = [
    ...
    '# Generated by setup-aca.ts — do not edit manually',
    '',
...
function preflight(): void {
  ...
  if (!azVersion) {
    console.error(pc.red('✗ Azure CLI (az) is not installed.'));
    console.error(pc.dim('  Install: https://aka.ms/install-azure-cli'));
    process.exit(1);
  }
  console.log(pc.green('✓') + ' Azure CLI found');

  // Check az login
  const azAccount = tryRun('az account show -o tsv --query name 2>/dev/null');
  if (!azAccount) {
    console.error(pc.red('✗ Not logged in to Azure. Run: az login'));
    process.exit(1);
  }
  console.log(pc.green('✓') + ` Azure account: ${pc.cyan(azAccount)}`);

  // Check gh CLI
  const ghVersion = tryRun('gh --version 2>/dev/null | head -1');
  if (!ghVersion) {
    console.error(pc.red('✗ GitHub CLI (gh) is not installed.'));
    console.error(pc.dim('  Install: https://cli.github.com'));
    process.exit(1);
  }
  console.log(pc.green('✓') + ' GitHub CLI found');

  // Check gh auth
  const ghUser = tryRun('gh api user -q .login 2>/dev/null');
  if (!ghUser) {
    console.error(pc.red('✗ Not logged in to GitHub. Run: gh auth login'));
    process.exit(1);
  }
...
async function selectAzureRegion(saved: Partial<Config>): Promise<string> {
  console.log(pc.bold('🌍 Azure Region\n'));
...
async function selectResourceGroup(saved: Partial<Config>): Promise<string> {
  ...
  const choices = [
    ...
    ...(existingGroups.length > 0 ? [new Separator('── existing ──')] : []),
    ...existingGroups
...
async function configureApp(saved: Partial<Config>): Promise<AppConfig> {
  console.log(pc.bold('⚙️  App Configuration\n'));
...
/**
 * Runs the full interactive prompt flow (repo, tag, region, app config).
...
async function main(): Promise<void> {
  ...
  console.log(pc.bold(pc.cyan('║   Azure Container Apps Setup Wizard      ║')));
  console.log(pc.bold(pc.cyan('╚══════════════════════════════════════════╝\n')));
